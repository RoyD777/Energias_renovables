...
para comparar la potencia eléctrica de cada una y la potencia total combinada.
""")

# --- Hidroeléctrica ---
st.header("Hidroeléctrica")
st.image("images/hidro.png", caption="Represa hidroeléctrica", use_container_width=False, width=300)
pot_hidro = st.slider("Potencial disponible (MW)", 0, 100, 80, step=1, key="ph")
...
pot_solar = st.slider("Potencial disponible (MW)", 0, 100, 30, step=1, key="ps")
...
pot_eolica = st.slider("Potencial disponible (MW)", 0, 100, 40, step=1, key="pe")
...
st.write(f"Potencia eléctrica: Hidroeléctrica = {energia_hidro:.2f} MW")
st.write(f"Potencia eléctrica: Solar = {energia_solar:.2f} MW")
st.write(f"Potencia eléctrica: Eólica = {energia_eolica:.2f} MW")
...
ax.set_ylabel("Potencia eléctrica aprovechada (MW)")
ax.set_title("Comparación de potencia eléctrica")
